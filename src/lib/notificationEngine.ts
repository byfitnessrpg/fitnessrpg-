...
export const NOTIFICATION_DATABASE: Record<NotificationCategory, NotificationCategoryInfo> = {
  bom_dia: {
    ...
    title: '🌅 Fitness Evolution: Novo Dia!',
    messages: [
      '💧 Bom dia, atleta. Sua jornada de hoje começou. Não esqueça de registrar sua primeira hidratação.',
      '⚡ Um novo dia significa uma nova oportunidade de evoluir. Comece registrando sua água.',
      ...
      '⚡ O Sistema iniciou uma nova meta diária. Sua primeira tarefa é cuidar do seu corpo.',
      '✨ O amanhecer traz novos desafios. Encha sua garrafa de água e prepare-se para treinar.',
      '💪 Proteja seu status de atleta: comece o dia ativando seu bônus de hidratação!'
    ]
  },
  meta_agua: {
    id: 'meta_agua',
    ...
    title: '💧 Fitness Evolution: Alerta de Água!',
    messages: [
      ...
      '🔋 Recarregue suas energias! A água é o combustível essencial para seu rendimento hoje.',
      '🔋 Alerta de energia baixa: beba água para manter seu corpo em alto desempenho.'
    ]
  },
  horario_treino: {
    id: 'horario_treino',
    ...
    title: '💪 Fitness Evolution: Hora do Treino!',
    messages: [
      '⚡ Sua meta de exercícios do dia está pronta.',
      '💪 Está na hora de evoluir.',
      '🔥 Seu treino está esperando por você.',
      '⚡ O Sistema preparou uma nova sequência de exercícios.',
      '📜 Hoje existe um desafio físico para concluir.',
      '⚡ Não deixe sua evolução para amanhã.',
      '⚡ Sua sessão de treinamento está pronta. Seus exercícios diários aguardam!',
      '💪 Nenhum atleta alcança o topo descansando. Venha completar sua rotina diária.'
    ]
  },
  missao_quase_fim: {
    id: 'missao_quase_fim',
    ...
    title: '⏳ Fitness Evolution: Tempo Esgotando!',
    messages: [
      '⚠️ Ainda dá tempo de completar seu treino.',
      '🔥 Não perca sua sequência de dias ativos!',
      '⏰ Suas metas diárias estão chegando ao fim.',
      '🏃 Apenas alguns minutos restantes para concluir o desafio de hoje.',
      '⚡ O Sistema ainda aguarda seu treino.',
      '🚨 Alerta: sua sequência de dias ativos corre risco se você não concluir o treino.',
      '⏳ O dia está chegando ao fim. Não deixe seu treino diário expirar!'
    ]
  },
  missao_concluida: {
    id: 'missao_concluida',
    ...
    title: '🏆 Fitness Evolution: Treino Cumprido!',
    messages: [
      '🏆 Excelente trabalho.',
      '⚡ Mais um treino concluído.',
      '🔥 Você ficou mais forte hoje.',
      '⭐ Cada treino concluído aproxima você do seu objetivo.',
      '📈 Seu progresso continua aumentando.',
      '🎉 Treino Concluído! Você ganhou XP e evoluiu seu nível de aptidão.',
      '💪 Incrível! Seus músculos se fortaleceram e seu condicionamento aumentou.'
    ]
  },
  reavaliacao: {
    id: 'reavaliacao',
    ...
    title: '📅 Fitness Evolution: Reavaliação Física!',
    messages: [
      '📈 Chegou o dia de medir sua evolução.',
      '⚡ O Sistema deseja analisar seu progresso.',
      '⚡ Você evoluiu nos últimos 10 dias?',
      '🔥 Hora de atualizar sua calibração de treino.',
      '💪 Descubra quanto você melhorou desde a última avaliação.',
      '📊 Hora do update de status! Vamos registrar suas novas marcas de força e peso.',
      '📊 Hora de ver seu progresso de aptidão física atualizado. Vamos avaliar?'
    ]
  },
  usuario_ausente: {
    id: 'usuario_ausente',
    ...
    title: '😴 Fitness Evolution: Sentimos Sua Falta',
    messages: [
      '👀 Sentimos sua falta.',
      '⚡ Sua jornada de evolução ainda continua.',
      '🔥 Não deixe sua evolução parar.',
      '⚡ O Sistema ainda acredita no seu potencial.',
      '💪 Volte e continue construindo sua melhor versão.',
      '💪 Seus objetivos aguardam seu retorno.',
      '⚡ Mantenha o foco nos seus objetivos. Dê o primeiro passo hoje e retome seus hábitos.',
      '🔋 Uma pequena pausa é normal, mas a consistência constrói campeões. Esperamos você!'
    ]
  }
};
...
const HISTORY_KEY = 'fitness_evolution_notification_history';
...
export const checkAndTriggerIntelligentNotifications = async (gameState: any) => {
  ...
  const isTriggeredToday = (category: NotificationCategory): boolean => {
    const lastDate = localStorage.getItem(`fitness_evolution_last_triggered_date_${category}`);
    return lastDate === todayStr;
  };

  const markAsTriggeredToday = (category: NotificationCategory) => {
    localStorage.setItem(`fitness_evolution_last_triggered_date_${category}`, todayStr);
  };
  ...
  const lastActiveStr = localStorage.getItem('fitness_evolution_last_active_timestamp');
  const nowMs = Date.now();
  if (lastActiveStr) {
    const lastActiveMs = parseInt(lastActiveStr, 10);
    const diffHours = (nowMs - lastActiveMs) / (1000 * 60 * 60);
    if (diffHours >= 48) {
      const lastAusenteTrigger = localStorage.getItem('fitness_evolution_last_triggered_date_usuario_ausente');
      ...
      if (isEligible) {
        ...
        localStorage.setItem('fitness_evolution_last_active_timestamp', nowMs.toString());
        return;
      }
    }
  }
